<template>
  <div
    ...
    :class="{
      'bg-gray-800 border-gray-900': modelValue.seen,
      'bg-amber-800 border-amber-900': !modelValue.seen,
      'opacity-80': modelValue.duration <= 60
    }"
  >
    ...
    <div class="p-5">
      <div class="flex flex-row text-sm font-bold text-white items-center pb-4">
        <div class="basis-1/2">
          <span class="py-2 px-2 bg-black rounded-full">{{
            formatDuration(modelValue.duration)
          }}</span>
          <span class="ml-2 py-2 px-3 bg-black rounded-full"
            >{{ abbreviateNumber(modelValue.viewCount) }} views</span
          >
        </div>
        <div class="basis-1/2 text-right flex items-center justify-end">
          ...
          <button
            class="bg-white text-black font-bold py-2 px-2 rounded-full text-center"
            @click="markWatchLater(!modelValue.watchLater)"
          >
      ...
      <h5 class="mb-2 text-md font-bold tracking-tight text-white">
        <a
          class="mr-2 text-blue-200"
          :href="'https://www.youtube.com/channel/' + modelValue.channelId"
          >{{ modelValue.channelTitle }}</a
        >
        <a :href="'https://www.youtube.com/watch?v=' + modelValue.youtubeId">{{
          modelValue.title
        }}</a>
      ...
      <p
        ...
        class="mb-3 overflow-hidden h-14 text-sm font-normal text-gray-200"
      >
